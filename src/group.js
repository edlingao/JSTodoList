class Group {
  ...
  editActivity(id, title, description, important = false, complete = false) {
    let newActivity = this._edit(
      this.speceficActivity(id),
      title,
      description,
      important,
      complete
    ...
    this.activities = this.activities.filter(activity => {
      if (activity.id == id) return newActivity;
      else return activity;
    });
  }
  editGroup(id, groupContinaer, groupName) {
    const newGroup = this._editGroup(
      groupContinaer.speceficActivity(id),
      groupName
    );
    this.activities = this.activities.filter(activity => {
      if (activity.id == id) return newGroup;
      else return activity;
    });
  }
  existsGroup(groupName) {
    ...
    if (!existing) return groupName;
    else return this._existsGroup(groupName);
  }
